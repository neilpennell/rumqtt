...
impl EventLoop {
    ...
    pub async fn poll(&mut self) -> Result<Event, ConnectionError> {
        ...
        match self.select().await {
            Ok(v) => Ok(v),
            Err(e) => {
                warn!("poll: got an error from select {:?}", &e);
